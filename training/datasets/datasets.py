...
class RLDSBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 1. 提取主视角 (注意这里的 [0], 说明是取窗口的第一帧)
        image = Image.fromarray(rlds_batch["observation"]["image_primary"][0])
        
        # print(rlds_batch["observation"].keys())
        
        # 2. 提取腕部视角 (新增逻辑)
        # 检查 observation 中是否有 image_wrist
        image_wrist = None
        ...
        # 3. 返回包含 image_wrist 的字典
        return dict(
            proprio=rlds_batch["observation"]['proprio'],
            action=action,
            image=image,             # 主视角
            image_wrist=image_wrist, # 腕部视角 (如果没有则为 None)
            lang=lang, 
