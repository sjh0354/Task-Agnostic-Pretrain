...
def pick_and_place_carrot_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    """
    针对 WidowX 250s 的优化转换函数。
    1. 截取前7维数据 (去除 redundant 的 fingers)
    2. 确保 proprio 和 action 对齐
    """
    
    # === 1. 处理 Observation (状态) ===
    # 你的 raw data 是 9维，我们只取前 7 维
    # [waist, shoulder, elbow, forearm_roll, wrist_angle, wrist_rotate, gripper]
    # 这样模型只需要处理 7 个数字
    raw_state = trajectory["observation"]["state"]
    trajectory["observation"]["proprio"] = raw_state[..., :7]

    # === 2. 处理 Action (动作) ===
    # 同样只取前 7 维作为动作标签
    raw_action = trajectory["action"]
    trajectory["action"] = raw_action[..., :7]

    # === 3. 处理 Language Instruction ===
    if "natural_language_instruction" in trajectory["observation"]:
        trajectory["language_instruction"] = trajectory["observation"]["natural_language_instruction"]
    else:
        # 如果没有指令，必须填充，否则 DataLoader 会报错
        # 注意：这里假设 batch 维度存在
        batch_size = tf.shape(trajectory["action"])[0]
        trajectory["language_instruction"] = tf.fill((batch_size,), "put the carrot on the plate")
...
def pick_and_place_carrot_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "natural_language_instruction" in trajectory["observation"]:
        ...
        # 注意：这里假设 batch 维度存在
        batch_size = tf.shape(trajectory["action"])[0]
        trajectory["language_instruction"] = tf.fill((batch_size,), "pick up the carrot")
